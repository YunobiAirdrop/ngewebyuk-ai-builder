...
// 1. Daftar Folder Lengkap Sesuai Struktur Proyek
const directories = [
  'config',
  'middleware',
  'services',
...
// 2. Daftar File beserta Isinya (Real Production Code)
const files = {
  '.github/workflows/main.yml': `name: Node.js CI
...
*.log`,
  ...
  'database/schema.sql': `-- Schema Produksi Database SQLite NgeWebYuk AI Builder
...
<head>
  <meta charset="UTF-8">
  <title>Login Admin - NgeWebYuk</title>
  <link rel="stylesheet" href="/css/style.css">
  <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-slate-950 text-white flex justify-center items-center min-h-screen">
  <form action="/admin/login" method="POST" class="bg-slate-900 p-8 rounded-xl border border-slate-800 space-y-4 w-96">
    <h1 class="text-2xl font-bold text-center">Admin Panel</h1>
    <input type="text" name="username" placeholder="Username" class="w-full p-2.5 bg-slate-800 border border-slate-700 rounded text-sm focus:outline-none">
    <input type="password" name="password" placeholder="Password" class="w-full p-2.5 bg-slate-800 border border-slate-700 rounded text-sm focus:outline-none">
    <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-500 p-2.5 rounded text-sm font-semibold transition">Login</button>
  </form>
</body>
</html>`,
...
<head>
  <meta charset="UTF-8">
  <title>Dashboard - NgeWebYuk Admin</title>
  <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-slate-950 text-white p-8">
  <h1 class="text-3xl font-bold">NgeWebYuk AI Admin Dashboard</h1>
  <script src="/js/admin.js"></script>
</body>
</html>`,

  'views/error.ejs': `<!DOCTYPE html>
<html lang="id">
<head>
  <meta charset="UTF-8">
  <title>Error - NgeWebYuk</title>
  <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-slate-950 text-white min-h-screen flex flex-col justify-center items-center p-4">
  <div class="bg-slate-900 border border-slate-800 p-8 rounded-xl max-w-md text-center">
    <h1 class="text-4xl font-extrabold text-red-500 mb-2">404</h1>
    <p class="text-slate-300 mb-6"><%= typeof message !== 'undefined' ? message : 'Terjadi Kesalahan Sistem' %></p>
    <a href="/" class="bg-indigo-600 hover:bg-indigo-500 px-4 py-2 rounded text-sm font-medium transition">Kembali ke Beranda</a>
  </div>
</body>
</html>`,

  'public/css/style.css': `/* Custom Global Stylesheet */
body {
  font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, sans-serif;
}`,

  'public/js/admin.js': `// Admin Dashboard Script Engine
...
// 4. Eksekusi Pembuatan File
Object.entries(files).forEach(([filepath, content]) => {
  fs.writeFileSync(path.join(__dirname, filepath), content);
  console.log(`✅ File dibuat: ${filepath}`);
});

console.log('\n🎉 SELURUH STRUKTUR PROYEK (TERMASUK MAIN.YML) BERHASIL DIBUAT!');
